"""Typed contract for random candidate selection."""
...
class RandomSelectInput(BaseModel):
    ...
    seed: int = Field(description="Deterministic random seed for selection")


class RandomSelectOutput(BaseModel):
    model_config = ConfigDict(extra="forbid")

    selected_ids: list[str] = Field(description="Selected candidate IDs")
